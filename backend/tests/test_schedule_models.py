...
def _create_rule(db_session) -> ShiftRule:
    rule = ShiftRule(
        code="rule-broadcast", name="广播规则",
        cycle_days=6, start_date="2026-06-01", persons_per_cell=2,
    )
    db_session.add(rule)
    db_session.commit()
    return rule
...
def _create_rule_version(db_session, rule: ShiftRule) -> ShiftRuleVersion:
    v = ShiftRuleVersion(
        rule_id=rule.id, version_no=1,
        cycle_days=rule.cycle_days, start_date=rule.start_date,
        persons_per_cell=rule.persons_per_cell, snapshot={"days": []},
    )
    db_session.add(v)
    db_session.commit()
    return v
...
def _build_ms(db_session, org, rule, rv, status="draft") -> MonthlySchedule:
    ms = MonthlySchedule(
        org_unit_id=org.id, rule_id=rule.id, rule_version_id=rv.id, status=status,
    )
    db_session.add(ms)
    db_session.commit()
    return ms
...
class TestMonthlySchedule:
    ...
    def test_org_unit_fk(self, db_session) -> None:
        ...
        rule = _create_rule(db_session)
        rv = _create_rule_version(db_session, rule)
        ms = _build_ms(db_session, org, rule, rv)
    ...
    def test_invalid_org_unit_raises(self, db_session) -> None:
        ...
        rv = _create_rule_version(db_session, rule)
        ms = MonthlySchedule(
            org_unit_id=99999, rule_id=rule.id, rule_version_id=rv.id,
        )
        db_session.add(ms)
    ...
    def test_invalid_rule_raises(self, db_session) -> None:
        ...
        rv = _create_rule_version(db_session, rule)
        ms = MonthlySchedule(
            org_unit_id=org.id, rule_id=99999, rule_version_id=rv.id,
        )
        db_session.add(ms)
        with pytest.raises(IntegrityError):
            db_session.commit()

    def test_unique_org(self, db_session) -> None:
        """同一机房不能有两份活跃排班"""
        org = _create_org(db_session)
        rule = _create_rule(db_session)
        rv = _create_rule_version(db_session, rule)
    ...
    def test_unique_org_soft_deleted(self, db_session) -> None:
        """软删除后可以创建同一机房的新排班"""
        org = _create_org(db_session)
        rule = _create_rule(db_session)
        rv = _create_rule_version(db_session, rule)
        ...
        db_session.commit()
        ms2 = MonthlySchedule(
            org_unit_id=org.id, rule_id=rule.id, rule_version_id=rv2.id,
        )
        db_session.add(ms2)
        db_session.commit()
...
class TestScheduleDay:
    ...
    def test_create_with_defaults(self, db_session) -> None:
        ...
        ms = _build_ms(db_session, org, rule, rv)
        day = ScheduleDay(
            schedule_id=ms.id, duty_date=date(2026, 7, 1), weekday=2,
        )
        db_session.add(day)
        db_session.commit()
    ...
    def test_with_holiday(self, db_session) -> None:
        ...
        ms = _build_ms(db_session, org, rule, rv)
        day = ScheduleDay(
            schedule_id=ms.id, duty_date=date(2026, 10, 1), weekday=3,
            is_legal_holiday=True, holiday_name="国庆节",
        )
        db_session.add(day)
        db_session.commit()
...
class TestScheduleShiftPerson:
    ...
    def _setup_shift(self, db_session):
        org = _create_org(db_session)
        rule = _create_rule(db_session)
        rv = _create_rule_version(db_session, rule)
        ms = _build_ms(db_session, org, rule, rv)
    ...
    def test_create_with_defaults(self, db_session) -> None:
        org, shift = self._setup_shift(db_session)
    ...
    def test_manual_source_type(self, db_session) -> None:
        ...
        sp = ScheduleShiftPerson(
            schedule_shift_id=shift.id, person_id=person.id,
            position_no=2, source_type="manual", remark="手动调整",
        )
        db_session.add(sp)
        db_session.commit()
