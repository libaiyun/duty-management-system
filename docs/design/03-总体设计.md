...
| 班次规则 | 班次、排班规则、规则绑定 | 班次规则页 | 班次、排班规则、规则明细 |
| 排班管理 | 月度排班生成、调整、发布、锁定 | 月度排班页、排班明细页 | 月度排班、排班日、排班班次 |
...
erDiagram
    ...
    shift_rule ||--o{ shift_rule_item : contains
...
- `monthly_schedule` 是月度排班主表，`schedule_day` 是日期，`schedule_shift` 是某日某班次，`schedule_shift_person` 是班次人员。
...
| --- | --- | --- |
| id | bigint PK | 班次 ID |
| code | varchar(64) | early/middle/night |
| name | varchar(64) | 早班/中班/晚班 |
...
| --- | --- | --- |
| id | bigint PK | 规则 ID |
| org_unit_id | bigint | 适用台站或机房 |
| code | varchar(64) | 规则编码 |
| name | varchar(128) | 规则名称 |
| station_type | varchar(64) | station_broadcast/station_satellite |
| persons_per_shift | int | 每班人数，广播发射台主城区为 2 |
| rule_type | varchar(32) | broadcast_fixed/custom |
...
| id | bigint PK | 明细 ID |
| rule_id | bigint | 所属规则 |
| group_type | varchar(32) | night_early_group/middle_group |
| sequence_no | int | 顺序 |
| shift_code | varchar(32) | early/middle/night/rest |
...
| rule_id | bigint | 使用规则 |
| status | varchar(32) | not_generated/draft/published/adjusted/locked/cancelled |
| generated_at | timestamptz | 生成时间 |
| published_at | timestamptz | 发布时间 |
| locked_at | timestamptz | 锁定时间 |
| remark | varchar(500) | 备注 |

唯一索引：`uk_monthly_schedule_org_month(org_unit_id, year_month, deleted_at)`。
...
索引：`idx_actual_duty_month_person(duty_date, actual_person_id)`，`idx_actual_duty_shift(schedule_shift_id)`。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/v1/schedules` | 查询月度排班列表 |
| POST | `/api/v1/schedules/generate` | 生成月度排班草稿 |
| GET | `/api/v1/schedules/{id}` | 排班详情 |
| GET | `/api/v1/schedules/{id}/days` | 查询日班次明细 |
...
| `schedule:monthly:generate` | 生成排班 |
...
| 月初排班提醒 | 每月 20 日 09:00 | 提醒下月排班未生成的机房主任 |
...
- 晚早组：晚班 -> 早班 -> 晚班 -> 早班，休息两轮。
- 中班组：中班 -> 中班，休息两轮；或中班 -> 休息一轮。
- 每班双岗值，按人员 `rotation_order` 生成。
- 自动生成结果允许管理员手工调整，调整写日志。
- 发布后生成实际值班初始记录。

### 17.2 实际值班同步策略

- 排班发布时：为每个排班人员生成 `actual_duty`，来源为 `schedule`。
...
| 报表 | 来源 | 输出 |
| --- | --- | --- |
| 月度值班表 | schedule_shift、schedule_shift_person | Excel，日期/星期/早中晚班 |
