...
| 班次规则 | 班次定义（按机房）、排班规则表格、规则版本 | 班次规则页 | 班次定义、排班规则、规则版本 |
| 排班管理 | 排班规则配置、发布、排班查看 | 排班规则页、排班表 | 排班规则、排班日、排班班次 |
| 换班管理 | 换班申请、对方确认、主任审批、生效 | 换班申请页 | 换班申请、审批记录 |
| 请假管理 | 公休假、事假、病假申请和审批 | 请假申请页、请假记录页 | 请假申请、审批记录 |
| 顶班管理 | 顶班安排（合并入请假记录页）、顶班人确认 | 我的顶班页、请假记录页 | 顶班安排、顶班确认 |
| 审批中心 | 待办、已办、审批流转 | 审批中心页 | 审批任务、审批记录 |
| 实际值班 | 汇总排班和业务变更后的值班口径 | 实际值班页 | 实际值班记录 |
| 退费管理 | 餐补退费、节假日加班费退费、汇总导出 | 退费管理页 | 退费批次、退费明细 |
...
`shift_def` 定义机房可用的班次类型，与机房绑定。表格列数和标题由此表动态决定。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | bigint PK | 班次 ID |
| org_unit_id | bigint | 所属机房 ID（FK org_unit.id） |
...
> **设计修订：** 新增 `org_unit_id`，班次定义按机房区分。不同机房可有不同班次配置。详见 `06-设计修订.md`。
...
| id | bigint PK | 标准 ID |
| org_unit_id | bigint | 所属机房 ID（FK org_unit.id） |
| meal_early | numeric(12,2) | 早班餐补，默认 10 |
| meal_middle | numeric(12,2) | 中班餐补，默认 10 |
| meal_night | numeric(12,2) | 晚班餐补，默认 14 |
| meal_refund | numeric(12,2) | 餐补退费，默认 4 |
| holiday_overtime | numeric(12,2) | 节假日加班费/人/班，默认 150 |
| holiday_refund | numeric(12,2) | 节假日退费，默认 56 |

> **设计修订：** 节假日全局统一（无 org_unit_id）。新增 `refund_standard` 表替代原 system_config 中的散落配置，按机房维护，首次进入预填充默认值。详见 `06-设计修订.md`。
...
- 数据范围：按本人、本机房、全部组织过滤数据。一人一机房，登录后自动确定。
...
| 机房副主任 | 所属机房 | 与机房主任权限完全一致 |
| 排班管理员 | 所属机房 | 生成、调整、发布、导出排班 |
| 财务/统计人员 | 所属机房 | 退费计算、考勤导出、报表导出 |
| 系统管理员 | 所选机房 + 全局 | 账号、角色、组织、配置、日志、备份；通过顶部切换器选择机房 |
...
| 本机房 | `org_unit_id = current_user.room_id` |
| 全部 | 不附加组织范围过滤（仅系统管理员在全局页面） |

> **设计修订：** 一人一机房，删除台站级数据范围规则。系统管理员通过顶部切换器选择机房后，机房相关页面的数据范围限定为所选机房。全局页面不受限制。
...
| 中班餐补 | 10 元 | 按机房可配置 |
| 晚班餐补 | 14 元 | 按机房可配置 |
| 餐补退费 | 4 元 | 按机房可配置，晚班退给中班 |
| 节假日加班费 | 150 元/人/班 | 按机房可配置，法定节假日 |
| 节假日退费 | 56 元 | 按机房可配置，150 * 3 / 8 取整 |

> **设计修订：** 费用标准按机房维护（见 `refund_standard` 表），首次进入某机房时预填充以上默认值。

### 12.3 配置管理原则

- 费用标准存入 `refund_standard` 表，按机房维护；首次进入某机房时预填充默认值。
- 配置变更必须写操作日志。
...
| code | 文案 |
| --- | --- |
| `ADMIN_NO_ROOM_SELECTED` | 请在顶部选择机房后再操作。 |
